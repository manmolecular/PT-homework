...
Для работы с проектом предпочтительно использование `pyenv`-среды с python версии `3.6.5` или новее. Все необходимые pip-зависимости указаны в файле `requirements.txt`, который находится в корне репозитория.  
  
Также для корректной работы `pytest` тестов необходимо установить пакет `python3-pymysql`:
```
sudo apt-get install python3-pymysql
```
...
### Стиль
Формат кода в проекте должен соответствовать [PEP 8](https://www.python.org/dev/peps/pep-0008/)
...
*Замечание: количество цифр в названии и общий формат не имеют значения - любой другой числовой формат будет распознан. Имя может не содержать в себе `_`, но следование общему формату тестов контролей желательно.*
...
### База данных
Используется SQLite база данных. Для удобного взаимодействия и контроля данных рекомендуется установить [DB Browser for SQLite](http://sqlitebrowser.org/)
